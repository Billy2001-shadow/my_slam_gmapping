...
MySlamGMapping::MySlamGMapping():
    ...
    private_nh_("~"), 
    scan_filter_sub_(NULL),  //订阅激光点云消息
    scan_filter_(NULL), 
    transform_thread_(NULL) //发布转换关系的线程
{
    seed_ = time(NULL);  //高斯噪声的随机数种子
    init();
}
...
void MySlamGMapping::startLiveSlam()
{
...

    {
        ...
        scan_filter_sub_ = new message_filters::Subscriber<sensor_msgs::LaserScan>(node_, scan_topic_, 5);//用message_filters库来订阅scan_topic_
        ...
        scan_filter_->registerCallback(boost::bind(&MySlamGMapping::laserCallback, this, _1)); //注册一个回调函数laserCallback，当接收到激光数据时，将调用该回调函数进行处理。boost::bind函数用于将MySlamGMapping类的成员函数laserCallback与当前对象进行绑定，以便在回调函数中可以访问类的成员变量和方法。
...
void MySlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan) //这里的scan是odom坐标系下的？ TODO
{
    //===========================激光雷达数据运动畸变处理部分===============================
...
bool MySlamGMapping::initMapper(const sensor_msgs::LaserScan& scan)
{
    ...
    std::cout << " x = " << initialPose.x << " y = " << initialPose .y << " theta = " << initialPose .theta  << std::endl; //TODO
    //为gmapping算法设置各种参数
...
void MySlamGMapping::publishLoop(double transform_publish_period)
{
    ...
    while(ros::ok())
    {
        publishTransform();  //TODO:从TF关系中顺便发布一个map坐标系下base_link的轨迹？
        r.sleep();
    }
}
